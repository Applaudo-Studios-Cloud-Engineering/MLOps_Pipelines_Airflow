...
prepare_submission = KubernetesPodOperator(dag=dag, 
    ...
    cmds=["python3", "-c"],
    arguments=["from pipelines.titanic import prepare_submission; prepare_submission('submodules/classification_projects/titanic_challenge/data/test.csv', '/tmp/clean.csv', '/tmp/feature.csv', '/tmp/submission.csv')"],
    executor_config = resource_config,
    namespace="default",
    image="ghcr.io/anggutie-dev/dags:latest",
